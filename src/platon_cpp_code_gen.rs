...
pub fn render_verification_key_from_default_template<T : Engine, E:RenderG1AffineToHex<T> + RenderG2AffineToHex<T> + Engine>(vk: &VerificationKey<T, PlonkCsWidth4WithNextStepParams>, render_to_path: &str) {
    // let template = include_str!("../template.sol");
    let template = PLATON_CPP_CONTRACT_TEMPLATE;
    render_verification_key_from_template::<T, E>(vk, template, render_to_path);
}
